...
! 1-file name 2-Date
      ...
      real :: DATA(700,3), DAT(700,3),  limit, mean
...
! open error asci file
      open(99,file=trim(outdir)//'/error_ascii.txt',form='formatted',position='append')
      write(99,'(a30)') '#########################################'
      write(99,'(a35)') infile
...
!!! SPIKES
               ...
               do nn = 1, jj
                  if (nn.eq.1) then ! the first point of the profile
                     ...
                     if (abs(DAT(nn-1,2)-DAT(nn,2)) .lt. 10 .and. abs(DAT(nn,2)-DAT(nn+1,2)).lt.10) then
                        j = j + 1
                        DATA(j,1) = DAT(nn,1)
                        DATA(j,2) = DAT(nn,2)
                        DATA(j,3) = DAT(nn,3)
...
!!!! STABILITY CHECK
               ...
               if (( j .eq. 0 )) then
                  ...
                  if (holes .eq. 1) then
                     ...
                     write(99,'(a54)') 'data lack in the thermocline, profile has been deleted'
                  else
                     kmss=M
                     kmss1=kmss
                     do k=kmss,1,-1
                        if(X(k,2).gt.35. .or. X(k,2).lt.0.0 .or. X(k,3).gt.45. .or. X(k,3).lt.0.0)then
                           kmss1 = k-1
                        endif
                     enddo
                     if(kmss1.lt.kmss) print*,'The profile is shorter. New levels are: ', kmss1 
                     kmss=kmss1
                     if(kmss.lt.1) then
                     ...
                     do k=1,kmss-1
                        if(X(k,1).lt.100. .and. (X(k+1,1)-X(k,1)).gt.40.) then
                           print*,'Profile rejected: levels ',X(k,1),'-',X(k+1,1),' missing.'
                           stop
                        endif
                     enddo
                     kkk = 0
! convert into potential temperature
               ...
               write(99,'(a70)') 'no quality control on date or position, profile skipped ',infile
      ...
      if (nvals .gt. 0)then
         do k=1,nvals
            if (dpta(k).lt.1000) then
               ...
               indx, para(k), lona(k), lata(k), dpta(k), tima(k), vala(k), erra(k), inoa(k)
            endif
         enddo
      endif
      close(12)
