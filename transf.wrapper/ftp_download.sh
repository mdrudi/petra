...
#### Change the following two lines when you install the script in an other machine
#locUser=`whoami`
...
for filelist in `find $PackSection -name "pack_ftp_download_*" -print | sort`; do
            ...
            if [ $? -eq 0 ]; then
               ...
               if [ "$buff_line_filename" != "$filename" ]; then
                  ...
                  if [ _$dim_remote == _$dim_local ]; then
                     ...
                     ncftpget -f $cfgfile -z -V -o useCLNT=0,useFEAT=0 ${destdir}/ $file
                     ncres=$? 
                     if [ $ncres -ne 0 ]; then
