...
#echo XXXX $PackSection
#echo
#echo 
#echo

for filelist in `find $PackSection -name "pack_ftpXXXX_upload_*" -print | sort`; do
         ...
         for file in `cat $filelist`; do
            if [ ! -f $file ]; then
               ...
               if [ $? -eq 0 ]; then
                  ...
                  buff_line=`ncftpls -f $cfgfile -l -a -o useCLNT=0,useFEAT=0 ftp://${desthost}:2120/${destdir}/$filename`
                  ...
                  if [ _$dim_remote == _$dim_local ]; then
                    ...
                    ### ncftpput -f $cfgfile -T .upl. -z -m -V -U 022 -y -o useCLNT=0,useFEAT=0 ${destdir}/ $file
                     ncftpput -P 2120 -f $cfgfile -T .upl. -t5 -z -m -y -o useCLNT=0,useFEAT=0 ${destdir}/ $file
                    # ncftpput -P 2120 -u phys_a -p p17dd155084A$ -T . cmems-med-mfc.eu ${destdir}/ $file
		     ncres=$? 
                     if [ $ncres -ne 0 ]; then
